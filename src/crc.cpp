...
extern unsigned char crc8(unsigned char* array, unsigned char i)
{
	unsigned char crc = 0;

	while (i--) { crc = CRC_Array[crc^*array++]; }
	return crc;
}
...
extern unsigned int crc16(volatile unsigned char* array, unsigned char size)
{
    ...
    while (size--)
        crc = (crc >> 8) ^ Crc16Table[(crc & 0xFF) ^ *array++];
